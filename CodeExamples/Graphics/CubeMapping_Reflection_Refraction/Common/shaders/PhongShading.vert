...
File Name: PhongShading.vert, implementation of phongShading, calculates
...
void main()
{
  ...
  fragNormal = normalize(tempNorm.xyz);

  gl_Position = (ProjTransform * vertexTransform) * vec4( modelPosition, 1.0f );
  position = ((inverse(ViewTransform) * vertexTransform) * vec4( modelPosition, 1.0f )).xyz;
