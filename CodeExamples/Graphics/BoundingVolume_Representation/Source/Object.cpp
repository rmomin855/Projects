...
Camera camera(glm::vec3(8350.0f, 39751.0f, 39000.0f));
...
//all uniforms needed for phong model
void Object::SetUniforms(GLuint programID)
{
  ...
  if (vTransformLoc >= 0)
  {
    glUniform3f(vTransformLoc, camera.cam_position.x, camera.cam_position.y, camera.cam_position.z);
  }
