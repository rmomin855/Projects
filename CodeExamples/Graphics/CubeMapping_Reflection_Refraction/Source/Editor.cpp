...
void Editor::ReloadShaders()
{
  ...
  curr->skyBox = LoadShaders("../Common/shaders/Skybox.vert",
...
void Editor::EnvironmentMenu(const ImVec2& screenSize)
{
  if (ImGui::Button("Use Reflection"))
  {
    environment->mapping = environmentEntity::reflection;
  }
  if (ImGui::Button("Use Refraction"))
  {
    environment->mapping = environmentEntity::refraction;
  }
  if (ImGui::Button("Use Mixed (Schlick’s method)"))
  {
    environment->mapping = environmentEntity::mix;
  }

  ImGui::InputFloat("Refraction Index", &environment->refectionIndex, 0.1f);

  environment->refectionIndex = environment->refectionIndex > 100.0f ? 100.0f : environment->refectionIndex;
  environment->refectionIndex = environment->refectionIndex < 1.0f ? 1.0f : environment->refectionIndex;

  ImGui::InputFloat("Interpolation Factor", &interpolationFactor, 0.01f);

  interpolationFactor = interpolationFactor > 1.0f ? 1.0f : interpolationFactor;
  interpolationFactor = interpolationFactor < 0.0f ? 0.0f : interpolationFactor;

  ImGui::InputFloat("FresnelPower", &environment->fresnal, 0.1f);
  environment->fresnal = environment->fresnal < 0.0f ? 0.0f : environment->fresnal;
}