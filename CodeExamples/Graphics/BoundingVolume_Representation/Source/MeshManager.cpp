...
Mesh* MeshManager::LoadMeshAssImp(const std::string& filename)
{
  Assimp::Importer importer;
  const aiScene* model = importer.ReadFile(filename, aiProcess_Triangulate | aiProcess_GenSmoothNormals | aiProcess_JoinIdenticalVertices);

  if (model)
  {
    Mesh* mesh = new Mesh();
    mesh->min = { INT_MAX, INT_MAX, INT_MAX };
    mesh->max = { INT_MIN, INT_MIN, INT_MIN };

    unsigned int numVertices = model->mMeshes[0][0].mNumVertices;
    aiVector3D* vertices = model->mMeshes[0][0].mVertices;
    aiVector3D* normals = model->mMeshes[0][0].mNormals;

    for (unsigned int i = 0; i < numVertices; i++)
    {
      mesh->vertex.push_back(glm::vec3(vertices[i].x, vertices[i].y, vertices[i].z));
      mesh->vertexNormals.push_back(glm::vec3(normals[i].x, normals[i].y, normals[i].z));

      mesh->min = glm::min(mesh->min, mesh->vertex[i]);
      mesh->max = glm::max(mesh->max, mesh->vertex[i]);

      mesh->centroid += mesh->vertex[i];
    }

    mesh->centroid /= numVertices;

    unsigned int numFaces = model->mMeshes[0][0].mNumFaces;
    aiFace* faces = model->mMeshes[0][0].mFaces;
    std::vector<GLuint> indices;
    ...
    SetUpModelBuffers(mesh->vertex, indices, mesh);

    Node* node = new Node(mesh->min, mesh->max, true, mesh);
    mesh->node->n_center = (mesh->min + mesh->max) * 0.5f;
    mesh->node->n_radius = glm::length((mesh->max - mesh->min) * 0.5f);
